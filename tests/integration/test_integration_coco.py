...
def test_coco_integration_coco(data_dir: Path) -> None:
    ...
    assert summary["ap_50"] == pytest.approx(0.697863184, 1e-8)
    assert summary["ap_75"] == pytest.approx(0.571587401, 1e-8)
    assert summary["mean_ap_sizes"]["small"] == pytest.approx(0.593280034, 2e-3)
    assert summary["mean_ap_sizes"]["medium"] == pytest.approx(0.557948082, 1e-8)
    assert summary["mean_ap_sizes"]["large"] == pytest.approx(0.489363210, 1e-8)
